...
function Items:AddSheckleTargetBox(SheckleBoxConfig)
    SheckleBoxConfig = SheckleBoxConfig or {}
    SheckleBoxConfig.Title = SheckleBoxConfig.Title or "Target Sheckles"
    SheckleBoxConfig.MinPlaceholder = SheckleBoxConfig.MinPlaceholder or "50m"
    SheckleBoxConfig.MaxPlaceholder = SheckleBoxConfig.MaxPlaceholder or "200m"
    SheckleBoxConfig.MinDefault = SheckleBoxConfig.MinDefault or ""
    SheckleBoxConfig.MaxDefault = SheckleBoxConfig.MaxDefault or ""
    SheckleBoxConfig.OnMinChange = SheckleBoxConfig.OnMinChange or function(val) end
    SheckleBoxConfig.OnMaxChange = SheckleBoxConfig.OnMaxChange or function(val) end

    local configKey = "SheckleTargetBox_" .. SheckleBoxConfig.Title
    local shouldSave = SheckleBoxConfig.Save ~= false

    local State = { Min = SheckleBoxConfig.MinDefault, Max = SheckleBoxConfig.MaxDefault }
    if shouldSave and ConfigData[configKey] ~= nil and type(ConfigData[configKey]) == "table" then
        State.Min = ConfigData[configKey].Min or State.Min
        State.Max = ConfigData[configKey].Max or State.Max
    end

    local SheckleBoxFunc = { Value = State }
    ...
    Block.BackgroundTransparency = 0.965
    Block.Size = UDim2.new(1, 0, 0, 40)
    Block.LayoutOrder = 1
    Block.Name = "SheckleBoxBlock"
    Block.Parent = Root
    ...
    BlockPad.PaddingTop = UDim.new(0, 8)
    BlockPad.PaddingBottom = UDim.new(0, 8)
    BlockPad.PaddingLeft = UDim.new(0, 8)
    BlockPad.PaddingRight = UDim.new(0, 8)
    BlockPad.Parent = Block

    local BlockList = Instance.new("UIListLayout")
    BlockList.Padding = UDim.new(0, 6)
    BlockList.SortOrder = Enum.SortOrder.LayoutOrder
    BlockList.Parent = Block

    local BlockTitle = Instance.new("TextLabel")
    BlockTitle.Font = Enum.Font.GothamBold
    BlockTitle.Text = SheckleBoxConfig.Title
    BlockTitle.TextColor3 = GuiConfig.Color
    BlockTitle.TextSize = 13
    BlockTitle.TextXAlignment = Enum.TextXAlignment.Left
    BlockTitle.BackgroundTransparency = 1
    BlockTitle.Size = UDim2.new(1, 0, 0, 14)
    BlockTitle.LayoutOrder = 0
    BlockTitle.Parent = Block

    local function MakeRow(labelText, placeholderText, defaultText, layoutOrder, onChangeCb, isMin)
        local Row = Instance.new("Frame")
        Row.BackgroundTransparency = 1
        Row.Size = UDim2.new(1, 0, 0, 40)
        ...
        TextBox.FocusLost:Connect(function()
            if isMin then State.Min = TextBox.Text else State.Max = TextBox.Text end
            Persist()
            onChangeCb(TextBox.Text)
        end)

        return TextBox
    end

    local MinBox = MakeRow("Value Range Min", SheckleBoxConfig.MinPlaceholder, State.Min, 1, SheckleBoxConfig.OnMinChange, true)
    local MaxBox = MakeRow("Value Range Max", SheckleBoxConfig.MaxPlaceholder, State.Max, 2, SheckleBoxConfig.OnMaxChange, false)

    function SheckleBoxFunc:Set(minVal, maxVal, noSave)
        State.Min = tostring(minVal or "")
        State.Max = tostring(maxVal or "")
        MinBox.Text = State.Min
        MaxBox.Text = State.Max
        if not noSave then Persist() end
    end

    ResizeRoot()

    if SheckleBoxConfig.MinDefault ~= "" then SheckleBoxConfig.OnMinChange(State.Min) end
    if SheckleBoxConfig.MaxDefault ~= "" then SheckleBoxConfig.OnMaxChange(State.Max) end

    if shouldSave then
        Elements[configKey] = SheckleBoxFunc
    end
    RegisterSearch({ label = SheckleBoxConfig.Title, tab = TabConfig.Name, kind = "SheckleTargetBox", switch = SearchSwitch })

    CountItem = CountItem + 1
    return SheckleBoxFunc
end
